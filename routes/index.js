...
const ensureLoggedIn = EnsureLoggedIn();

const fetchExer = (req, res, next) => {
    db.all('SELECT * FROM exercises WHERE owner_id = ?', [
        req.user.id
    ], function (err, rows) {
        if (err) { return next(err); }

        const exer = rows.map(function (row) {
            return {
                id: row.id,
                title: row.title,
...
router.post('/', ensureLoggedIn, function (req, res, next) {
    ...
    db.run('INSERT INTO exercises (owner_id, title, completed) VALUES (?, ?, ?)', [
        req.user.id,
        req.body.title,
...
router.post('/:id(\\d+)', ensureLoggedIn, function (req, res, next) {
    ...
    if (req.body.title !== '') { return next(); }
    db.run('DELETE FROM exercises WHERE id = ? AND owner_id = ?', [
        req.params.id,
        req.user.id
    ], function (err) {
        if (err) { return next(err); }
    ...
    db.run(' UPDATE exercises SET title = ?, completed = ? WHERE id = ? AND owner_id = ?', [
        req.body.title,
...
router.post('/toggle-all', ensureLoggedIn, function (req, res, next) {
    db.run('UPDATE exercises SET completed = ? WHERE owner_id = ?', [
        req.body.completed !== undefined ? 1 : null,
        req.user.id
    ], function (err) {
        if (err) { return next(err); }
